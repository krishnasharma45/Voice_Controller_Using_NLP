...
void Audio::Record() {
  ...
  int bitPerSample = i2s->GetBitPerSample();  // Renamed for clarity

  if (bitPerSample == 16) {
    for (int j = 0; j < wavDataSize/dividedWavDataSize; ++j) {
      size_t bytes_read = 0;
      i2s_read(I2S_NUM_0, (void*)i2sBuffer, i2sBufferSize / 2, &bytes_read, portMAX_DELAY);  // Fixed i2s_read usage
    ...
    for (int j = 0; j < wavDataSize/dividedWavDataSize; ++j) {
      size_t bytes_read = 0;
      i2s_read(I2S_NUM_0, (void*)i2sBuffer, i2sBufferSize, &bytes_read, portMAX_DELAY);  // Fixed i2s_read usage
      for (int i = 0; i < i2sBufferSize / 8; ++i) {
